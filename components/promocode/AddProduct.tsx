...
import { OrderItem } from "../orders/create/types";
      ...
      return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {categories
            ?.filter((category) =>
              searchTerm
                ? category.name_uz?.toLowerCase() === searchTerm?.toLowerCase()
                : category
            )
            ?.map(
              (category) =>
                (category?.children.length > 0 ||
                  ...
                  >
                    <h3 className="text-md text-[#272d37]">
                      {category.name_uz}
                    </h3>
                  </div>
                )
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogTrigger asChild>
        <div className="flex items-center cursor-pointer text-[#038AFF]">
          <Plus className="h-4 w-4 mr-2" color="#038AFF" /> Taom qo'shish
        </div>
      </DialogTrigger>
