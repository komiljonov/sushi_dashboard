  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {order.iiko_order_id && <div className="flex items-center space-x-2">
          <Hash className="h-4 w-4" />
          <Label>Iiko ID:</Label>
        ...
        <div className="space-y-4">
          {items?.map((item) => (
            <div key={item.id} className="flex items-center space-x-4">
              {item.product?.image ? (
                <Image
                  src={(item?.product?.image as IFile).file}
  ...
  const { mutate: cancel, isPending: cancelPending } = useMutation({
    ...
    onError: () => {
      toast({
        ...
        description:
          "Buyurtmani bekor qilishni iloji bo'lmadi. Tizimda hatolik.",
      });
    },
  });
